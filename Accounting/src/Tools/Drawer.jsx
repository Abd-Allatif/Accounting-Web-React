...
function CustomDrawer({ isOpen, toggleDrawer }) {
  ...
  const [drawerSide, setDrawerSide] = useState(i18n.dir() === 'rtl' ? 'right' : 'left');
  ...
  const languages = [
    { code: "en", lang: "English" }, { code: "ar", lang: "عربي" }]
        ...
        <div className='Drawer' onClick={toggleDrawer(false)} onKeyDown={toggleDrawer(false)}>
          <div className="BluredScreen">
            <div className="LanguageSelector">
              {languages.map((lng) => {
                return <button className={lng.code === i18n.language ? "selected" : "btn"} key={lng.code} onClick={() => changeLanguage(lng.code)}>{lng.lang}</button>
              })}
...
const DrawerContent = styled.div`
  .Drawer{
    ...
    transform: translateX(${props => props.isOpen ? '0' : props.drawerSide === 'left' ? '-100%' : '100%'});
